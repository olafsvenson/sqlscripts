/*---------------------------------------------------------------------
  ...
  EXEC beta_lockinfo @archivemode = 60

  SELECT *
  FROM [master].[guest].[beta_lockinfo] l
  --WHERE l.rsctype='OBJECT' AND locktype ='X'
  --ORDER BY rowno
  order by now desc
  
  
  $Header: /WWW/sqlutil/beta_lockinfo_2012-14.sp 4     17-04-08 18:58 Sommar $
  ...
  $History: beta_lockinfo_2012-14.sp $
 * 
 * *****************  Version 4  *****************
 * User: Sommar       Date: 17-04-08   Time: 18:58
 * Updated in $/WWW/sqlutil
 * Added two more wait types filtered out when querying
 * sys.dm_os_waiting_tasks.
 * 
 * *****************  Version 3  *****************
 * User: Sommar       Date: 16-05-29   Time: 21:05
 * Updated in $/WWW/sqlutil
 * 1) Fixed potential overflow problem with trn_since.
 * 2) Get inputbuffer for all requests when MARS is in use.
 * 3) Slight performance improvement when getting query text and plans.
 * 4) Use try_convert when getting query plans, and use the fallback only
 * for the ones where we were not able to get the plan at once.
 *
 * *****************  Version 2  *****************
 * User: Sommar       Date: 16-04-25   Time: 0:03
 * Updated in $/WWW/sqlutil
 * Error message for version check had the wrong SQL Server version.
 *
 * *****************  Version 1  *****************
 * User: Sommar       Date: 16-04-25   Time: 0:00
 * Created in $/WWW/sqlutil
 * Branching off a version specific for SQL 2012/2014.
 *
 * *****************  Version 20  *****************
...
IF @version < '11'
   RAISERROR('This version of beta_lockinfo requires SQL Server 2012 or later', 16, 127)
go
-- Create shell.
...
WHILE 1 = 1
BEGIN
   FETCH C1 INTO @spid, @request_id
   IF @@fetch_status <> 0
      BREAK

   BEGIN TRY
      INSERT @inputbuffer(eventtype, params, inputbuffer)
         EXEC sp_executesql N'DBCC INPUTBUFFER (@spid, @request_id) WITH NO_INFOMSGS',
                            N'@spid int, @request_id int', @spid, @request_id
   ...
   END TRY
   BEGIN CATCH
      INSERT @inputbuffer(inputbuffer, spid, request_id)
         VALUES('Error getting inputbuffer: ' + error_message(),
                @spid, @request_id)
   END CATCH
END
...
--------------------------------------------------------------------
-- Get query plans. As with the query text, we could be blocked. A
-- second complication is that the XML plan may exceed the nesting
-- level supported by the XML data type. As with the query text we
-- fall back on a loop, but if the XML fails to convert we want an
-- error message why the condition to enter the fallback is different.
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN TRY
            -- This time without the error message, so we can catch the error.
            UPDATE @procs
